...
for HORIZON in 80
do
FILENAME=test_output/ensemble_check_success_rate/$(whoami)_$(date +%y_%m_%d_%H_%M_%S)_horizon_${HORIZON}_solver_${SOLVER_TYPE}_env_${ENV_NAME}_cpu_True
touch $FILENAME
for SEED in 1
# NMP Failure cases
...
do
    python -m nmp.test_run --cpu --verbose --obstacles_type handcraft:hardest --exp-name $LOG_DIR/params.pkl --seed $SEED --horizon $HORIZON --solver_type $SOLVER_TYPE $ENV_NAME #>> $FILENAME
done
done
