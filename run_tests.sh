...
# Shaoxiong laptop
LOG_DIR=/home/yaosx/Desktop/EECS598_006_final_project/sshape_boxes_global_1024/seed1

# SOLVER_TYPE=RL
# ENV_NAME=SShape-Boxes-64Pts-Rays-v0
ENV_NAME=SShape-Boxes-1024Pts-SurfaceNormals-v0
OBS_TYPE=handcraft:set_1
...
for HORIZON in 5
do
FILENAME=test_output/early_stop_trick/$(whoami)_$(date +%y_%m_%d_%H_%M_%S)_horizon_${HORIZON}_solver_${SOLVER_TYPE}_env_${ENV_NAME}_obs_${OBS_TYPE}
touch $FILENAME
for SEED in {1..20}
# NMP Failure cases
...
do
if [ $SOLVER_TYPE == 'single_RL_RRT' ]
then
    python -m nmp.test_run --cpu --obstacles_type $OBS_TYPE --exp-names $LOG_DIR/itr_980.pkl --seed $SEED --horizon $HORIZON --solver_type RL_RRT $ENV_NAME >> $FILENAME
else
    python -m nmp.test_run --obstacles_type $OBS_TYPE --exp-names $LOG_DIR/itr_900.pkl --exp-names $LOG_DIR/itr_920.pkl --exp-names $LOG_DIR/itr_940.pkl --exp-names $LOG_DIR/itr_960.pkl --exp-names $LOG_DIR/itr_980.pkl --seed $SEED --horizon $HORIZON --solver_type $SOLVER_TYPE $ENV_NAME 
fi
done
done
done
