...
for HORIZON in 80
do
FILENAME=test_output/0808test/$(whoami)_$(date +%y_%m_%d_%H_%M_%S)_horizon_${HORIZON}_solver_${SOLVER_TYPE}_env_${ENV_NAME}_obs_${OBS_TYPE}
touch $FILENAME
for SEED in {1..100}
# NMP Failure cases
...
then
    python -m nmp.test_run --obstacles_type $OBS_TYPE --exp-names $LOG_DIR/itr_980.pkl --seed $SEED --horizon $HORIZON --solver_type RL_RRT $ENV_NAME >> $FILENAME
else
    python -m nmp.test_run --obstacles_type $OBS_TYPE --exp-names $LOG_DIR/itr_900.pkl --exp-names $LOG_DIR/itr_920.pkl --exp-names $LOG_DIR/itr_940.pkl --exp-names $LOG_DIR/itr_960.pkl --exp-names $LOG_DIR/itr_980.pkl --seed $SEED --horizon $HORIZON --solver_type $SOLVER_TYPE $ENV_NAME >> $FILENAME
fi
done
done
done
