...
imsize48_default_architecture = dict(conv_args=dict(
    kernel_sizes=[5, 3, 3],
    n_channels=[16, 32, 64],
    strides=[3, 2, 2],
),
...
imsize48_default_architecture_with_more_hidden_layers = dict(conv_args=dict(
    kernel_sizes=[5, 3, 3],
    n_channels=[16, 32, 64],
    strides=[3, 2, 2],
),
...
imsize84_default_architecture = dict(conv_args=dict(
    kernel_sizes=[5, 5, 5],
    n_channels=[16, 32, 32],
    strides=[3, 3, 3],
),
...
class ConvVAE(GaussianLatentVAE):
    def __init__(
        ...
        self.encoder = encoder_class(**conv_args,
        ...
        self.decoder = decoder_class(**deconv_args,
                                     ...
                                     paddings=np.zeros(len(deconv_args['kernel_sizes']),
                                                       dtype=np.int64),
                                     hidden_init=hidden_init,
                                     **deconv_kwargs)
    ...
    def decode(self, latents):
        ...
        if self.decoder_distribution == 'bernoulli':
            ...
            return torch.clamp(decoded, 0,
                               1), [torch.clamp(decoded, 0, 1),
